...
| `noise_pos` | cell `(x, y)` or `null` | the site's noise broadcast on any loud event (§9: gunfire, alarm, body found), copied onto every actor **within `NOISE_RADIUS = 12` cells (Chebyshev, walls ignored)** | `noise_heard`, `move_to_blackboard` | **turn** — cleared when the actor reaches it or its Pass ends |
